...
describe("Transaction screen with data for WITHDRAW/BUSD", () => {
  beforeEach(() => {
  ...
  it("should match data", () => {
    ...
    expect(transactionViewModel.isDeposit).toBe(false)
    expect(transactionViewModel.isRepay).toBe(false)
    expect(transactionViewModel.isWithdraw).toBe(true)
    ...
    expect(transactionViewModel.tokenBalance.toString()).toBe(item.supply.toString())
    expect(transactionViewModel.titleBasedOnType).toBe("transaction.withdraw")
    expect(transactionViewModel.buttonTitleBasedOnType).toBe("transaction.withdraw $0.00")
  ...
  it('supply should be enabled, and supply balance is greater than input', () => {
    const transactionScreen = render(<Transaction/>)
    const input = transactionScreen.getByLabelText('cost-input') as any
  ...
  it('max is set', () => {
    ...
    fireEvent.click(max)
    expect(transactionViewModel.inputValue).toBe(item.supply.toString())
  ...
  it('set max and clear with input', () => {
    ...
    fireEvent.change(input, {target: {value: '13.827778533603640197'}})
    expect(transactionViewModel.inputValue).toBe(item.supply.toString())
    expect(transactionViewModel.isMaxValueSet).toBe(true)
  ...
  it('should handle max on swap', () => {
    ...
    fireEvent.click(swap)
    expect(transactionViewModel.isMaxValueSet).toBe(true)
    expect(transactionViewModel.inputValue).toBe(Big(item.supply).toString())
  ...
  it('should return tokens max value on swap before transaction', () => {
    ...
    expect(transactionViewModel.inputValue).toBe(item.supply.toString())
    expect(transactionViewModel.isMaxValueSet).toBe(true)
    expect(transactionViewModel.getInputValueForTransaction).toBe(item.supply.toString())
